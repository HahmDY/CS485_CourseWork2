...
switch MODE
    ...
    case 'Caltech' % Caltech dataset
        ...
        desc_sel = single(vl_colsubset(cat(2,desc_tr{:}), 10e4)); % Randomly select 100k SIFT descriptors
        
        % K-means clustering
        numBins = 256;
        vocab = vl_kmeans(desc_sel, numBins, 'verbose', 'algorithm', 'elkan', 'MaxNumIterations', 50);
        
        % Build kd-tree for fast indexing
        ...
        if showImg
            ...
            suptitle('Image representations: 256-D histograms');
        end
        
        for c = 1:length(classList)
            for i = 1:length(imgIdx_tr)
                % Each image patch is mapped to a codeword in the codebook
                b = double(vl_kdtreequery(kdtree, vocab, single(cat(2,desc_tr{c,i})), 'MaxComparisons', 50));
                % The image is represented by the histogram of the codewords
                data_train(cnt1,:) = [hist(b,numBins)/length(b) c];
                
                % Visualise histogram
                if i < 6 & showImg
                    subaxis(length(classList),5,cnt2,'SpacingVert',0,'MR',0);
                    bar(data_train(cnt1,1:end-1));
                    cnt2 = cnt2+1;
                    axis([1 256 0 0.04]);
                    drawnow;
                end
...
switch MODE
    case 'Caltech'
        ...
        for c = 1:length(classList)
            for i = 1:length(imgIdx_te)
                b = double(vl_kdtreequery(kdtree, vocab, single(cat(2,desc_te{c,i})), 'MaxComparisons', 50)) ;
                data_query(cnt,:) = [hist(b,numBins)/length(b) c];
                cnt = cnt+1;
                
                % Visualise histogram
                if i < 6 & showImg
                    subaxis(length(classList),5,cnt2,'SpacingVert',0,'MR',0);
                    bar(data_query(cnt2,1:end-1));
                    cnt2 = cnt2+1;
                    axis([1 256 0 0.04]);
                    drawnow;
                end
